...
export MAX_K8S_VER := $(or $(MAX_VERSION),max$(shell jq -r .maxk8s.version < version.json)).k8s
export BIZ_K8S_VER := $(or $(BIZ_VERSION),biz$(shell jq -r .bizk8s.version < version.json)).k8s
...
export IPD_K8S_VER := $(or $(IPD_VERSION),ipd$(shell jq -r .ipdk8s.version < version.json)).k8s
...
public-build: ## 公网构建开源版镜像
	/bin/bash -x ./hack/make-rules/public-build.sh $(APP_NAME) $(OPEN_VER) $(PHP_VER) $(MYSQL_VER) "linux/amd64,linux/arm64" "Dockerfile"
